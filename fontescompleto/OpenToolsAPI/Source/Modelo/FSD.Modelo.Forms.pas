...
uses
  ...
  FSD.Modelo.FormCadastro.Creator, FireDAC.Phys.SQLiteDef;
...
begin
  {$IF CompilerVersion <= 26}
  FDConn.Params.Values['Database'] := edtDataBase.Text;
  {$ELSE}
...
begin
  ...
  try
    for i := 0 to Pred(tabelas.Count) do
      TFSDModeloFormCadastroCreator.New(GetNamespace, tabelas[i]);
  finally
    tabelas.Free;
  end;
end;
